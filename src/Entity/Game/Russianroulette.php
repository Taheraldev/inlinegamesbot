        ...
        if (isset($arg)) {
            ...
            if ($data['cylinder'][$arg - 1] === 'X') {
                ...
                if ($data['current_turn'] == 'X') {
                    ...
                    $gameOutput = Emoji::skull() . ' <b>' . __("مات {PLAYER}! (مطرود)", ['{PLAYER}' => '</b>' . $this->getUserMention($data['settings']['O']) . '<b>']) . '</b>' . PHP_EOL;
                    $gameOutput .= Emoji::trophy() . ' <b>' . __("{PLAYER} فاز!", ['{PLAYER}' => '</b>' . $this->getUserMention($data['settings']['X']) . '<b>']) . '</b>';
